...
\begin{center} % center the text only for the title page
	\vspace*{200pt}
	\textbf{\huge{Comparison of three source types in two and three layer models in Salvus}}\\
	\vspace{75pt}
...
\noindent The two layer model shows much less reflection at the snow-air interface, compared to the three layer model, for example in figure \ref{fig:ac_layered_t56}. 


\subsection{Elastic}

\noindent The observations made about the speed of the wave propagation are also true for the elastic case with a vector source. The two-layer case is shown in figure \ref{fig:el_snowair_v2d_all}, the three layer case in \ref{fig:el_layered_v2d_all}. The waves from the vector point 2D source also do not propagate through the air in both cases, in the three layer case in figure \ref{fig:el_layered_v2d_all} also does not show propagation through the earth layer. 
...
\noindent While the two layer case shows distinct patterns close to the source, the three layer case in figure \ref{fig:el_layered_v2d_all} shows wavefronts of more extreme velocities propagating at later timesteps in \ref{fig:el_layered_v2d_t42} and \ref{fig:el_layered_v2d_t56}. In the two layer case, propagation velocities show no clear areas with extremes, velocities have the same magnitude everywhere. In addition, while the two layer case in \ref{fig:el_snowair_v2d_t42}  and \ref{fig:el_snowair_v2d_t61} shows patterns at the same x-location as the source, the same timesteps in the three-layer case shown in figures  \ref{fig:el_layered_v2d_t42} and \ref{fig:el_layered_v2d_t56} show a blindness at the x-location of the source, there is no velocity field above and below the source. 

\subsubsection{Moment Tensor Source}
\label{el_tensor}

\noindent The moment tensor source representing a skier sliding on the snow also does not show any wave propagation through the air layer for both the two layer case in figure \ref{fig:el_snowair_mom_all} and three layer in \ref{fig:el_layered_mom_all}. Here, the difference in distance of the wavefield propagation between the two and three layer case is not as remarkable as the other source types, at timestep 6, figures \ref{fig:el_snowair_mom_t3} and \ref{fig:el_layered_mom_t3} show similar propagation distances in the horizontal direction. 
...
\noindent All of these equations show a dependance of wave speed to density, so it would make sense that in a material of lower density like air, the wave speed is much lower in amplitude and is scaled down. 
